...
<div wire:poll.1000ms="tick"
     class="w-full rounded-xl border border-amber-400/40
            bg-linear-to-r from-amber-950/70 via-amber-900/40 to-amber-950/70
            px-5 py-4 backdrop-blur-sm shadow-lg shadow-amber-900/20
        ...
        <div class="flex items-start gap-3">
            ...
            <div>
                ...
                <p class="mt-0.5 text-xs text-amber-200/70">
                    Silakan cek kotak masuk email Anda dan klik tautan verifikasi.
                    Tautan berlaku selama <strong>5 menit</strong>.
                </p>

                {{-- Success message --}}
                @if($sent)
                    <p class="mt-1.5 flex items-center gap-1.5 text-xs font-medium text-emerald-400">
                ...
                {{-- Error message --}}
                @if($errorMessage)
                    <p class="mt-1.5 text-xs font-medium text-red-400">
                        ⚠ {{ $errorMessage }}
                    </p>
                @endif
            </div>
        ...
        <div class="shrink-0 pl-11 sm:pl-0">
            @if($cooldownSeconds > 0)
                <button disabled
                        class="inline-flex cursor-not-allowed items-center gap-1.5 rounded-lg
                               border border-amber-400/20 bg-amber-500/10 px-4 py-2 text-xs
            ...
            @else
                <button wire:click="resend"
                        ...
                        class="inline-flex items-center gap-2 rounded-lg border border-amber-400/30
                               bg-amber-500/20 px-4 py-2 text-xs font-semibold text-amber-300
